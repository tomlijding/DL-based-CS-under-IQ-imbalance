...
for noise_level in noise_levels:
    ...
    OMP_noisy_losses.append(OMP_MSE)
    PSOMP_MSE = sum(sum((h-h_hat_psomp))**2)/len(y)
    PSOMP_noisy_losses.append(PSOMP_MSE)
...
for sensing_size in sensing_sizes:
    ...
    OMP_MSE = sum((h-h_hat_omp) ** 2)/len(y)
    OMP_sensing_size_losses.append(OMP_MSE)
    PSOMP_MSE = sum(sum((h-h_hat_psomp)) ** 2)/len(y)
    PSOMP_sensing_size_losses.append(PSOMP_MSE)
